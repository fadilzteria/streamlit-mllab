...
def explain_model(config, split="Train"):
    ...
    with open(fe_sets_filepath, 'r', encoding="utf-8") as file:
...
def show_sklearn_pd_ice_plots(ex_model, x_data, _features, cat_features, ice=False, target_idx=0):
    if ice:
        _features = [x for x in _features if x not in cat_features]

    # Create Subplots
    nrows, ncols = ((len(_features)-1)//3)+1, 3
    ...
    for i, feature in enumerate(_features):
        ...
        if ice:
            PartialDependenceDisplay.from_estimator(
                ex_model, x_data, [feature], kind="individual",
                random_state=42, ax=ax_temp, grid_resolution=50, target=target_idx
            )
        else:
            PartialDependenceDisplay.from_estimator(
    ...
    if (len(_features)-1) < (nrows*ncols):
        for j in range(len(_features), (nrows*ncols)):
...
def show_pd_plots(ex_model, shap_values, x_data, targets, imp_thres, ice):
    ...
    if len(targets) > 2:
        for i, target in enumerate(targets):
            ...
            # Features
            _features = list(shap_importance_df["Feature"])
            cat_features = x_data.select_dtypes(exclude=['int', 'float']).columns.tolist()

            # PD Plots
            show_sklearn_pd_ice_plots(ex_model, x_data, _features, cat_features, ice, target_idx=i)
    else:
        ...
        if len(shap_importance_df) > 0:
            ...
            # PD Plots
            show_sklearn_pd_ice_plots(ex_model, x_data, _features, cat_features, ice)
        else:
            st.warning("No Feature Importance")

# Easy-Difficult Samples
@st.cache_data
def show_easy_difficult_samples(
    df, model_names, target_column, str_pred, spec_model_name, target_name=""):
    ...
    if spec_model_name=="":
        sorted_rank = full_model_rank
    else:
        sorted_rank = f"{spec_model_name}_Worst{target_name}"
...
def extract_easy_difficult_samples(train_config, oof_df, spec_model_name="", fold=0, split="Train"):
    ...
    _features = [train_config["id"], "index", target_column]
    ...
    features_with_pred = _features.copy()
    ...
    if train_config["ml_task"]=="Classification":
        ...
        for target_name in target_names:
            ...
            features_with_tpred = _features.copy()
            ...
            show_easy_difficult_samples(
                target_df, model_names, target_column, str_pred,
                spec_model_name=spec_model_name, target_name=f"_{target_name}"
            )
    else:
        show_easy_difficult_samples(
            filtered_df, model_names, target_column, str_pred, spec_model_name
        )
