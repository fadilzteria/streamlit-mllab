            ...
            if "CatBoost" in model_name:
                ...
                for cat_feat in cat_cols:
                    if x_test[cat_feat].isnull().values.any():
                        x_test[cat_feat] = x_test[cat_feat].cat.add_categories("Missing")\
                            .fillna("Missing")
...
@st.cache_data
def ensembling(train_config, test_df, pred_df, model_name=""):
    ...
    if train_config["ml_task"]=="Classification":
        ...
        # Hard Classes
        ensembled_df[hard_target_name] = np.argmax(
            ensembled_df.iloc[:, 1:len(class_names)+1], axis=1
        )
        ensembled_df[hard_target_name] = ensembled_df[
            hard_target_name
        ].apply(lambda x: class_names[x])
...
def full_ensembling(test_config, train_config, test_df, pred_df):
    ...
    for model_name in methods:
        ensembled_df = ensembling(
            train_config, test_df, pred_df, model_name=model_name
        )
...
def eval_testing(test_config, train_config, ensembled_df):
    ...
    for model_name in methods:
        ...
        if train_config["ml_task"]=="Classification":
            model_proba_names = [
                col for col in ensembled_df.columns if model_name in col and "pred" not in col
            ]
