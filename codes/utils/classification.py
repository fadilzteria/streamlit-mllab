    ...
    for model_name in model_names:
        ...
        for n in range(1, nums+1):
            ...
            # Tree
                ...
                methods[model_n_name] = DecisionTreeClassifier(
                    max_depth=max_depth, random_state=42, class_weight='balanced'
                )
            elif model_name=="Extra Trees":
                max_depth = params[f"{model_n_key}_params_max_depth"]
                methods[model_n_name] = ExtraTreesClassifier(
                    max_depth=max_depth, random_state=42, class_weight='balanced'
                )
            elif model_name=="Random Forest":
                max_depth = params[f"{model_n_key}_params_max_depth"]
                methods[model_n_name] = RandomForestClassifier(
                    max_depth=max_depth, random_state=42, class_weight='balanced'
                )
                ...
                max_depth = params[f"{model_n_key}_params_max_depth"]
                methods[model_n_name] = GradientBoostingClassifier(
