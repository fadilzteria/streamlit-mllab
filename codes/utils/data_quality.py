...
def drop_unique_features(df, persisted_features=[]):
...
def check_duplicated_rows(df, drop_duplicated_rows=False, target_exist=None, spec_features=[]):
    ...
    if(target_exist):
        ...
        # Extract Index
        last_dup_no_feat_target = df_no_feat_target[df_no_feat_target.duplicated(subset=dup_columns)]
        dup_index = last_dup_no_feat_target.drop_duplicates(keep="last", subset=dup_columns).index.tolist()

        # Get All Duplicates
        dup_no_feat_target = df_no_feat_target[df_no_feat_target.duplicated(keep=False, subset=dup_columns)]
        st.write("Duplicated Rows for Dataframe Without Specific Features and Target:", dup_no_feat_target.shape[0])
...
def sampling_data(full_df, sample_size, id, group=None):
    ...
    if(group):
        ...
        for group_name in full_df[group].unique():
            ...
            group_sample_df = group_full_df.sample(frac=sample_ratio, random_state=42, ignore_index=True)
    ...
    sample_df = sample_df.sort_values(by=id, ignore_index=True)
