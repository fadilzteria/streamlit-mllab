...
def test_dataset():
    ...
    # Dataset Input
    at.selectbox("data_file").set_value("regress_train.csv").run()
    at.selectbox("_id").set_value("id").run()
    at.selectbox("_target").set_value("price").run()
    at.button[0].click().run()

    # Dataset Preprocessing
    at.text_input("dp_name").set_value("Regress Baseline").run()
...
def test_training():
    ...
    # Experiment
    at.text_input("exp_name").set_value("Regress Baseline").run()
    at.selectbox("dp_name").set_value("Regress Baseline").run()
    at.selectbox("ml_task").set_value("Regression").run()
    ...
    # Model
    model_names = ["Linear Regression", "Ridge Regression", "Lasso", "Elastic Net"]
    ...
    at.selectbox("best_value").set_value("Minimize").run()
...
def test_prediction():
    ...
    csv_files = ["regress_test_labels.csv", "regress_test_no_labels.csv"]
